...
export default function DashboardPage() {
            ...
            try {
                ...
                if (_active) {
                    if (profile && profile.cargo) {
                        _0x1s(profile.nome);
                        _0x2s(profile.cargo.trim().toUpperCase());
                    } else {
    ...
    const _se1 = _x('U1VQRVJWSVNPUl9FU1RPUVVF');        // SUPERVISOR_ESTOQUE
    const _se2 = _x('U1VQRVJWSVNPUiBFU1RPUVVF');        // SUPERVISOR ESTOQUE
    const _se3 = _x('U1VQRVJWSVNPUkVTVE9RVUU=');        // SUPERVISORESTOQUE
    const _se4 = _x('U1VQRVJWSVNPUg==');                // SUPERVISOR
    const _se5 = _x('U1VQRVJWSVNPUiBERSBFU1RPUVVF');    // SUPERVISOR DE ESTOQUE
    const _se6 = _x('U1VQRVJWSVNPUl9ERV9FU1RPUVVF');    // SUPERVISOR_DE_ESTOQUE

    // Mapeamento de permissões:
    // _0xvp inclui o Supervisor de Estoque para dar acesso ao Ponto, Pausas e Emergência
    const _0xvp = [_a, _g, _t, _gf, _ag1, _ag2, _ag3, _se1, _se2, _se3, _se4, _se5, _se6];
    const _0xvrf = [_a, _g, _t, _m, _gf, _se1, _se2, _se3, _se4, _se5, _se6];
    const _0xvhf = [_a, _g, _t, _m, _gf, _se1, _se2, _se3, _se4, _se5, _se6];
    const _0xvf = [_a, _g, _gf];
    const _0xvrh = [_a, _g, _ag1, _ag2, _ag3];
    const _0xve = [_a, _g, _t, _m, _gf, _e, _se1, _se2, _se3, _se4, _se5, _se6];
    ...
    return (
        <main className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f] font-sans antialiased flex flex-col lg:flex-row w-full selection:bg-black/5">
            ...
            <section className="flex-1 p-4 sm:p-6 md:p-10 max-w-[1400px] flex flex-col gap-6 w-full z-10 overflow-y-auto">
                ...
                {/* SEÇÃO PONTO (CONTROLE DE PONTO, PAUSAS E EMERGÊNCIA) */}
                {_0xvp.includes(_0x2r) && (
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5 w-full">
                            ...
                            {/* BOTÃO 2: /dashboard/ponto/pausas */}
                            ...
                            {/* BOTÃO 3: /dashboard/ponto/emergencia */}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3.5 w-full">
                        ...
                        {_0xvfnc.includes(_0x2r) && (
                            <Link href="/dashboard/funcionarios" className="bg-white border border-[#e5e5ea] hover:border-[#b4b4b9] p-5 rounded-2xl flex flex-col justify-between min-h-[140px] transition-all group shadow-[0_1px_3px_rgba(0,0,0,0.01)]">
                                ...
                                <div className="mt-4 leading-snug">
                                    <h3 className="text-xs font-bold tracking-tight text-[#1d1d1f] group-hover:opacity-70 transition-opacity">{_x('TGlzdGEgZGUgRnVuY2lvbnM=')}</h3>
